...
const FB_PAGE_ACCESS_TOKEN = 'EAACeJX7kxMcBAIcmyNYfqTRoyZBZBlYwYcqbHCl6MNjugIykz5nXU1oL7kO3fsxKWbAU2Qyp04B4HRJqeComlymR8y1UY49hZARQ9a0zGbVKCJMjooIOxI1jZCr0BEt5qQdIhR35AvUmoo0jbaXwZCSXPR7t3PyrClKeZBbXwrdAZDZD';
...
app.post('/news', (req, res) => {
  ...
  if (data.result.action === 'news_search') {
        const newsChannel = data.result.parameters['News'];
        const apiUrl = 'https://newsapi.org/v2/top-headlines?sources=' + newsChannel + '&apiKey=' + NEWS_API;
        ...
        request({
            ...
            const article = JSONbig.parse(body).articles;
           // console.log(allElements (article));
            return res.json({
                data:{
                   "facebook": { 
                      "attachment": {
 ...
 function allElements (article) {
 const elements = [];
 for(var i = 0; i < 5; i++) {

    const temp =  {
            "title": article[i].title,
            "image_url": article[i].urlToImage,
